...
function check_main_command($textcheck){
    ...
    $reply = array("ต้องการ เช็คตารางงาน \n คุณ $lineuser->name ตอนนี้ยังไม่มีงานให้นะคะ",
...
if($lineuser->lastcommand =='ช่วยเหลือ'){
    global $lineuser,$bot,$text;
        if($text){ 
            if($text =='ปล่อยสินค้า') {
                $bot->reply("กรุณารอสักครู่ เครื่องกำลังดำเนินการปล่อยสินค้า");
            }
                
            if($text =='Restart'){
                $bot->reply("กรุณารอสักครู่ รอประมาณ 5 นาที เครื่องกำลังทำการ Restart เครื่อง");
