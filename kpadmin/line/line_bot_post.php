...
$result =$bot->getUserProfile($_POST['userId']);  // ของตุล 

$line_profile = json_decode($result,true); // ของตุล 
//$line_display = urlencode($line_profile["displayName"]);
//$line_picture = $line_profile["pictureUrl"];

/*  Aut แก้ไขใหม่ */ 
$line_display = $_POST["displayName"];
$line_picture = $_POST['pictureUrl'];
...
if(true){ //$from_line){		// KEEP IN DB
   ...
   // $linechat->userId       = "U9db96fa969ea1a9da8f0350a82a5cf77";   // for Test
    $linechat->write();
    ...
    if(!$lineuser->displayname) {
        $lineuser->displayname  = $line_display;
        $lineuser->write();
    }
