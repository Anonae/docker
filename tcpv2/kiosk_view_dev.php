...
if($_POST['action']=='setupSLOT'){ 
	...
	if($_POST['kioskInventoryID']){
		...
		if($updateSLOT->productid!=NULL && $updateSLOT->status!=0){
			...
			$updateSLOT->price_retail = $_POST["retail"];
			$updateSLOT->capacity = $_POST["capacity"]; 
			$updateSLOT->current_qty = ($_POST["current_qty"]+$_POST["fill_qty"]); 
			$updateSLOT->status = 0;
...
if($_POST['action']=='delSLOT'){ 
	$updateDEL = new kiosk_inventory();
	$updateDEL->id = $_POST['kioskInventoryID']; 
	if($updateDEL->load()){ 
		// 1.Update Inventory
		$updateDEL->productid = 0;
		$updateDEL->current_qty = 0;
		$updateDEL->price = 0;
		$updateDEL->status = 1; //Status 1 ถือว่า ลบไป ไม่ใช้งาน
		$updateDEL->lastupdate = time(date('Y-m-d H:i:s'));
		$updateID = $updateDEL->write();
		// 2.Create Del Log
		if($updateID){
			$insertDEL = new kiosk_inventory_transactions();
			$insertDEL->kioskid = $_GET["kioskid"];
			$insertDEL->slotid = $_POST["slotid"];
			$insertDEL->productid = $_POST["productid"];
			$insertDEL->qty = $_POST['current_qty'];
			$insertDEL->status = 3; //Status 3 คือ สินค้าเก็บกลับ
			$insertDEL->refill = 0;
			$insertDEL->price = 0;
			$insertDEL->lastupdate = time(date('Y-m-d H:i:s'));
			$insertDEL->coin1B = 0; $insertDEL->coin2B = 0; $insertDEL->coin5B = 0; $insertDEL->coin10B = 0; $insertDEL->note20B = 0;
			$insertDEL->coinCashOut = 0; $insertDEL->coinCashTube = 0; $insertDEL->iot_transactions_id = 0;
			$insertDEL->write();
			$resultAction = 'updateDEL';
		}
	}
}  //////////////////////////////////////////////////////////////////////////////////
if($_POST['action']=='fillallSLOT'){ 
	if($_POST["superpass"] == '123456789'){ //ตรวจสอบ รหัสผ่านพิเศษ เป็น 123456789
		//Create Reset Point
		$createRESET = new kiosk_inventory_transactions();
		$createRESET->id = NULL; 
		$createRESET->kioskid = $_GET["kioskid"];
		$createRESET->slotid = 0;
		$createRESET->productid = 0;
		$createRESET->qty = 0;
		$createRESET->status = 3; //Status 3 คือ รีเซ็ตยอดเงิน
		$createRESET->refill = 0;
		$createRESET->price = 0;
		$createRESET->lastupdate = time(date('Y-m-d H:i:s'));
		$createRESET->coin1B = 0; $createRESET->coin2B = 0; $createRESET->coin5B = 0; $createRESET->coin10B = 0; $createRESET->note20B = 0;
		$createRESET->coinCashOut = 0; $createRESET->coinCashTube = 0; 
		$createRESET->iot_transactions_id = 0;
		$createRESET->write();
		//Update QTY All Slot in Kiosk
		$fetchSLOT = new kiosk_inventory();
		$fetchSLOT->kioskid = $_GET["kioskid"];
		$fetchSLOT->status = 0;
		$fetchSLOT->loadmany();
		if($fetchSLOT){ $forSLOT = $fetchSLOT->id; //vd($findSLOT); die();
			for($x=0; $x<count($forSLOT); $x++){
				$updateSLOT = new kiosk_inventory(); 
				$updateSLOT->id = $forSLOT[$x];
				$updateSLOT->load(); 
				//Load Then Update
				if($updateSLOT->productid != NULL){ 
					$fillQYT = ($updateSLOT->capacity - $updateSLOT->current_qty); 
					$updateSLOT->current_qty = $updateSLOT->capacity; // Max Slot 20 Unit
					$updateSLOT->write();
					//Update Fill QTY Transaction
					if($fillQYT != 0){
						$updateTRN = new kiosk_inventory_transactions();
						$updateTRN->id = NULL; 
						$updateTRN->kioskid = $_GET["kioskid"];
						$updateTRN->slotid = $updateSLOT->slotid;
						$updateTRN->productid = $updateSLOT->productid;
						$updateTRN->qty = $fillQYT;
						$updateTRN->status = 1; //Status 1 คือ เติมสินค้า
						$updateTRN->refill = 0;
						$updateTRN->price = 0;
						$updateTRN->lastupdate = time(date('Y-m-d H:i:s'));
						$updateTRN->coin1B = 0; $updateTRN->coin2B = 0; $updateTRN->coin5B = 0; $updateTRN->coin10B = 0; $updateTRN->note20B = 0;
						$updateTRN->coinCashOut = 0; $updateTRN->coinCashTube = 0; 
						$updateTRN->iot_transactions_id = 0;
						$updateTRN->write();
						$resultSuperpass = 'ผ่าน';
					}
				} //$updateID
				unset($fillQYT);
			}
		}
	} else { $resultSuperpass = 'รหัสไม่ถูกต้อง';}//ตรวจสอบ รหัสผ่านพิเศษ
} //////////////////////////////////////////////////////////////////////////////////
...
<div id="slotbox-wrapper" class="row" style="margin:0 -2px;">
    ...
    <div class="col-12" style="border-top:2px solid rgba(0,0,0,0.03); padding:10px 2px 10px 2px; display:table; width:100%;">
        ...
        <div class="btn-group" style="float:left; ">
            <button class="btn btn-default btn-sm btn-slotshow btn-info" style="font-weight:600; font-size:15px;" onClick="canvasGen(this)">
            	<i class="fa fa-fw fa-stack-overflow"></i> แสดงตามช่องเก็บสินค้า
            </button>
            <button class="btn btn-default btn-sm btn-kioskshow" style="font-weight:600; font-size:15px;" onClick="kioskGen(this)">
...
?>
    ...
    <div class="slotbox sr-only" id="loadSLOT_<?=$loadSLOT[slotid]?>">
        ...
        <div class="slotbox-content">
            ...
            <div class="product">
                ...
                <? }else{ ?>
                    ...
                    <? else: ?>
                        <img src="images/products/<?=$loadPRD->product_image?>" title="<?=$loadPRD->product_name?>" style="min-height:100px;" />
                    <? endif; ?>
                    <? if($loadSLOT[price_retail]!='0'): ?>
                    	<div class="price retail"><b><?=number_format($loadSLOT[price_retail],0)?></b></div>
                    <? else: ?>
