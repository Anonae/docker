...
//Transaction
$sqlA = "
	SELECT trn.productid, SUM(trn.price) AS sumsold, COUNT(trn.price) AS recordssold
	FROM redbull.kiosk_inventory_transactions AS trn
	WHERE trn.status = 0 AND trn.lastupdate BETWEEN {$dateStart} AND {$dateEnd} 
	";
	if($_GET['kioskid']){ $sqlA .= " AND trn.kioskid = {$kioskid}";}
$sqlA .= " GROUP BY trn.productid ORDER BY sumsold DESC";
$res = ROOT::query($sqlA);
                ...
                <div class="row">
                    ...
                    <div class="col-md-6 col-sm-6 col-xs-12" style="padding-top:10px;">
                        ...
                        <? 	$top5Amount = 0; 
								...
								//Chart Asset
								$chartsXAxis_Label[] = $usePRD->name[$bestSOLD[$x][productid]].' '; //Chart xAxis Label
						...
						?>
                            ...
                            <li id="rankPRD_<?=$bestSOLD[$x][productid]?>" style="position:relative;">
                            	<div class="rank <? echo 'rank'.($x+1);?>"><i class="fa fa-fw fa-certificate rankInner"></i> <b class="rankInner"><?=$x+1;?></b></div>
                                <? if($usePRD->img[$bestSOLD[$x][productid]]!=NULL): ?>
                                <div class="image">

                                    <? if(substr($usePRD->img[$bestSOLD[$x][productid]],0,4) == 'data'): ?>
                                    <img src="<?=$usePRD->img[$bestSOLD[$x][productid]]?>" class="img-responsive" style="max-height:60px;margin:0 auto;" />
                                	<? else: ?>
                                    <img src="images/products/<?=$usePRD->img[$bestSOLD[$x][productid]]?>" class="img-responsive" style="max-height:60px;margin:0 auto;" />
                                    <? endif; ?>
                                ...
                                <? endif; ?>
                                <div class="content">
                                    <div class="topic"><span style="color:#bbb; font-weight:300; font-size:13px;">(<?= $bestSOLD[$x][productid] ?>)</span> <?=$usePRD->name[$bestSOLD[$x][productid]]?></div>
            ...
            <div class="x_content"> 
				...
				<? 	$bestPRDINDEX = 0;
						...
						$bestPRDINDEX = ($bestSOLD[$x][sumsold]*100)/$overallSold;
							if($bestSOLD[$x][sumsold]!=0):
				?>
				<tr id="salesTR_<?=$bestSOLD[$x][productid]?>" class="<?=($onDateTimeStart>time())?'disabled':''?>">
                    <td style="text-align:left;">
                    	<span style="color:#bbb; font-weight:300;">(<?=$bestSOLD[$x][productid]?>)</span> <b style="font-weight:500; color:#000;"><?=$usePRD->name[$bestSOLD[$x][productid]]?></b>
                    </td>
                    ...
                    <td style="text-align:right; border-left:1px solid #ddd; <?=($bestSOLD[$x][recordssold]>0)?'font-weight: 700;':''?>">
                    	<a href="transaction_lists.php?productid=<?=$bestSOLD[$x][productid]?>&datefrom=<?=date('Y-m-d', $dateStart)?>&date=<?=date('Y-m-d', $dateEnd)?>">
