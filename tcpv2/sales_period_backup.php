...
$thisSQL = " SELECT * FROM kiosk_inventory_transactions";
	$thisSQL .= " WHERE status = 0 AND lastupdate BETWEEN {$dateStart} AND {$dateEnd}";
	if($_GET['kioskid']){ $thisSQL .= " AND kioskid = {$kioskid}";}
	if($_GET['productid']){ $thisSQL .= " AND productid = {$productid}";}
$thisSQL .= " ORDER BY id DESC";
$thisResult = ROOT::query($thisSQL);
while($thisTRN = mysqli_fetch_array($thisResult,MYSQLI_ASSOC)){ $loadTRN[] = $thisTRN; $sumPrice += $thisTRN[price]; }
            ...
            <div class="x_content"> 
				...
				<? 	$periodSoldACCUM = 0;
						for($x=0; $x < 24; $x++) : 
							...
							for($xd=0; $xd<=count($loadTRN); $xd++){ $totalSold += $loadTRN[$xd][price];
								if(date('H', $loadTRN[$xd][lastupdate]) == str_pad($x,2,"0",STR_PAD_LEFT) ) {
									$periodTSN[] = $loadTRN[$xd];
									$periodSoldSUM += $loadTRN[$xd][price]; 
								}
							}
